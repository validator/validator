...
public class EncodingDetectInputStream extends InputStream {
  ...
  private static final short[] detectProg = { 
    // number of bytes, bytes,..., loByteIndex, bytesPerChar, bomLength,
    4, 0x00, 0x00, 0xFE, 0xFF, 3, 4, 4, 
    4, 0xFF, 0xFE, 0x00, 0x00, 0, 4, 4,
    4, 0x00, 0x00, 0xFF, 0xFE, 2, 4, 4,
    4, 0xFE, 0xFF, 0x00, 0x00, 1, 4, 4,
    2, 0xFE, 0xFF,             1, 2, 2,
    2, 0xFF, 0xFE,             0, 2, 2,
    3, 0xEF, 0xBB, 0xBF,       0, 1, 3,
    4, 0x00, 0x00, 0x00, 0x3C, 3, 4, 0, 
    4, 0x3C, 0x00, 0x00, 0x00, 0, 4, 0,
    4, 0x00, 0x00, 0x3C, 0x00, 2, 4, 0,
    4, 0x00, 0x3C, 0x00, 0x00, 1, 4, 0,
    4, 0x00, 0x3C, 0x00, 0x3F, 1, 2, 0,
    4, 0x3C, 0x00, 0x3F, 0x00, 0, 2, 0, 
    4, 0x3C, 0x3F, 0x78, 0x6D, 0, 1, 0 
  //4, 0x4C, 0x6F, 0xA7, 0x94, 0, 1, 0,
  };
  ...
  public String detectEncoding() throws IOException {
    ...
    while (makeAvailable((chIndex + 1)*bytesPerChar + bomLength)) {
      char ch = (char)buf[start + bomLength +chIndex*bytesPerChar + loByteIndex];
	...
	b[i] = (char)buf[start + bomLength + i*bytesPerChar + loByteIndex];
      try {
	TextDecl decl = new TextDecl(b, 0, b.length);
	enc = decl.getEncoding();
      }
      catch (InvalidTokenException e) {
	throw new CharConversionException(Localizer.message("INVALID_TEXT_DECL"));
      }
    }
    ...
    if (enc == null) {
      if ((bytesPerChar == 2 && bomLength == 0) || bytesPerChar > 2)
